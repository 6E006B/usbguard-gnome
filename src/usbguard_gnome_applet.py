            ...
            if not device.is_allowed():
                if self.screensaver_active:
                    ...
                    if self.settings.get_hid_screenlock() and device.is_hid_only():
                        ...
                        notification = Notify.Notification.new(_("New keyboard attached"), _("Locking computer now. Enter your password to activate. If you did not attach a keyboard, check your computer for potentially malicious devices."), self.USBGUARD_ICON_PATH)
                        ...
                        notification.add_action('temp', _('Temporary'), self.on_temp_clicked, device)
                        notification.add_action('allow', _('Allow'), self.on_allow_clicked, device)
                        notification.add_action('block', _('Block'), self.on_block_clicked, device)
                        notification.add_action('default', _('default'), self.on_notification_clicked, device)
